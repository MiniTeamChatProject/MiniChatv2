...
func (uc *UserUseCase) Register(ctx context.Context, username, password, nickname string) error {
	uc.log.WithContext(ctx).Infof("Register a new user NOW : %s", username)

	// 1. 检查用户是否已存在 (这一步也可以在 Data 层做，但这里做逻辑更清晰)
	existing, _ := uc.repo.FindByUsername(ctx, username)
	if existing != nil {
		return errors.New("This user exists ALREADY !")
	}
...
func (uc *UserUseCase) Login(ctx context.Context, username, password string) (string, error) {
	uc.log.WithContext(ctx).Infof("The user trys to log in : %s", username)

	// 1. 查找用户
	user, err := uc.repo.FindByUsername(ctx, username)
	if err != nil {
		return "", errors.New("Error: the user does not exist or the data-base errs !") // 也可以返回 nil
	}
	if user == nil {
...
func (uc *UserUseCase) DeleteUser(ctx context.Context, id int64) error {
    uc.log.WithContext(ctx).Infof("Deleting user ID: %d", id)
...
func (uc *UserUseCase) UpdateProfile(ctx context.Context, id int64, nickname, email string) error {
    uc.log.WithContext(ctx).Infof("Information of the user %d is to be updated ", id)
